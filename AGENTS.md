...
- Run everything under the conda env `opennourish` (**Python 3.12**): `/home/markus/miniconda3/envs/opennourish/bin/python`. Conda `base` has no Flask and will fail at import. `opennourish-py39-backup` is the pre-move 3.9 clone; the 3.12 image was deployed and confirmed on TrueNAS 2026-09-23, so its rollback job is over and `conda env remove -n opennourish-py39-backup` is now safe.
...
- `upgrade-research/` — dependency/interpreter migration material, root-owned. `README.md` is the 2026-09-23 audit (its baseline figures describe the **pre-move** state) and `PLAN.md` is the live sequenced plan with current status. M0–M2, M2b, M4's CI step and deployment provenance landed 2026-09-23: env and Dockerfile on 3.12, `requirements.txt` generated from `requirements.in` (61 packages), the image's `typst` at 0.15.1 with label output verified pixel-identical, and the image deployed to TrueNAS with `BUILD_INFO` proving the running revision. M2's cold-volume boot was verified on a fresh checkout: the full `entrypoint.sh` path (USDA download, `usda_data.db` rebuild, migrations, all four seed steps) ran clean under 3.12. Pending: M3, M4's runtime/dev split, M5, and M6 — security hardening (CSRFProtect, mutating GETs, deploy secrets, the seed-admin default), opened 2026-09-23 now that the upgrade track is closed.
