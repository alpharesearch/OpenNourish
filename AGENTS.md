...
- `docker-compose.yml` publishes no port for the app; only `nginx` maps 80/443 and reaches the app over the compose network by service name. The `README` claim about `localhost:8081` is wrong. The project name is pinned by `name: opennourish`, so the local build is always `opennourish-opennourish-app` — the name `deploy_truenas.sh` tags and pushes. Without that key the name came from the checkout directory, so a clone in `~/opennourish-test` built images the script never tagged.
