...
- `models.py` — all 19 SQLAlchemy models and the two-bind layout, plus `_utcnow_naive()` — the private twin of `opennourish.time_utils.utcnow_naive()` that exists only because `opennourish/__init__.py:4` imports this module at module level.
...
- **User-entered text reaches Typst as escaped data, not as markup** (fixed 2026-09-23, PLAN.md M5). Every interpolation in `opennourish/typst_utils.py` now goes through one of two context-specific escapers; before that, the USDA path escaped only `*`, so a food named `Yogurt $5 pack` failed its own label with `unclosed dollar` (a 500) and a `"` in a UPC ended the `#ean13` string. `opennourish/AGENTS.md` owns which escaper applies where and why straight quotes are excluded. The accepted remainder: a line inside ingredients or instructions starting with `-`, `+`, `=`, or `|` still renders as a Typst list/heading/table marker — cosmetic, verified to compile on 0.15.1, no failure. Renderer upgrades are otherwise safe here — 0.13.1 → 0.15.1 was verified pixel-identical.
...
- `docker-compose.yml` publishes no port for the app; only `nginx` maps 80/443 and reaches the app over the compose network by service name. The `README` claim about `localhost:8081` is wrong. The project name is pinned by `name: opennourish`, so the local build is always `opennourish-opennourish-app` — the local name `deploy_truenas.sh` tags and re-publishes under the registry namespace. Without that key the name came from the checkout directory, so a clone in `~/opennourish-test` built images the script never tagged.
...
- **The pushed repository name is a contract with TrueNAS, not cosmetics.** `deploy_truenas.sh` pushes `<registry>/<TRUENAS_REGISTRY_NAMESPACE>/<name>` (default `library`) and prints matching `image:` lines. Reason: on 25.10 the *update available* badge for a custom app comes only from `image_updates_available` — middlewared's sweep (`app.image.op.check_update`, at Docker service start and every 86400 s, gated by `docker.config.enable_image_updates`) fetches `https://<registry>/v2/<image>/manifests/<tag>` **anonymously** with normal certificate validation and compares that digest against the local image's `RepoDigests`. Before asking, `normalize_reference` prepends `library/` to any single-component repository name — private registries included — so an unnamespaced push is probed at a path Distribution never had (measured on the production NAS at 25.10.6: 200 for the pushed name, 404 for the probed one). The 404 is logged and swallowed per image, the flag is never set, and deploys silently degrade to stop-container + delete-image by hand. Two more 25.10 facts: the probe never sends the Apps → Registries credentials (those are pull-only), so an htpasswd registry is a permanent 401; and a transport failure is not a `CallError`, so it aborts the entire sweep, not just that image. Applying an update is `app.pull_images` + redeploy against the same tag, refused while the app is stopped. Diagnostics live in `DEV-README.md` § "Step 2.2".
...
- `nginx/nginx.conf` sets `client_max_body_size 10m` at `http` level (2026-09-24). Its absence was a functional bug, not only a hardening gap: nginx's 1 MiB default answered the YAML food and recipe importers with 413 before the request reached Flask, and the app sets no `MAX_CONTENT_LENGTH`, so this directive is the only body cap in the stack. Validate a config change with `docker run --rm --add-host opennourish-app:127.0.0.1 -v "$PWD/nginx/nginx.conf":/etc/nginx/nginx.conf:ro <nginx-image> nginx -t` — the image's own entrypoint generates the self-signed certs `-t` needs and the host mapping satisfies the upstream lookup.
- `.dockerignore` carries two syntax traps that turn a pattern into a silent no-op: a trailing ` # comment` becomes part of the pattern, and **patterns are anchored at the context root** — `__pycache__/` matches `./__pycache__` only, so nested ones need `**/__pycache__/`. Both bit here: the shipped image used to contain `.kilocode/` 58 MB, `htmlcov/` 9.8 MB, `.kilo/` 6 MB, 22 nested `__pycache__` dirs with 275 stale `.pyc`, `.coverage`, `.ruff_cache/`, `QWEN.md` and `project_snapshot.txt`, so `/app` was 87 MB of which ~81 MB was junk. Fixed 2026-09-24: `/app` is now 6.0 MB and the image 347 MB (was 420 MB from this checkout, matching the 351 MB a clean clone built). Measure any new pattern by building and running `docker run --rm --entrypoint sh <image> -c 'du -sh /app'` — never trust the file by reading it. A clean build is otherwise byte-reproducible (measured 2026-09-25: two builds with identical `VCS_REF`/`BUILD_DATE` yield the same image ID and layer set), so a layer that churns on every build means the **context** changed — anything created inside the checkout before a build is copied in by `COPY . .`, which is how a scratch `DOCKER_CONFIG` directory made three layers differ per run until it moved under the ignored `htmlcov/`. `THIRD-PARTY-LICENSES.md` must **not** be excluded: it is the licence text the distributed image relies on for copyleft djlint.
...
- `deploy_truenas.sh` **sources** `.env` (`set -a; . ./.env; set +a`), which is why `.env` must stay a plain `KEY=value` file: it is executed, and values containing spaces need quoting. Until 2026-09-24 it did `export $(cat .env | grep -v '^#' | xargs)`, and the measured failure was worse than truncation — for any quoted value with a space bash set **nothing**, so a `TRUENAS_APP_PATH=/mnt/data pool/…` silently fell back to the script's hard-coded default path and that path went into the YAML for pasting. It no longer emits `FLASK_DEBUG`, which no code reads. Its closing `cat <<EOF` prints the generated TrueNAS custom-app YAML — **which is the deploy artifact itself, and its `SECRET_KEY`/`ENCRYPTION_KEY`/`MAIL_*` lines are load-bearing** (see Work Guidance: mail comes from the environment until the DB says otherwise). **The owner has ruled that this print stays exactly as it is (2026-09-23):** do not silence, redact, or redirect it to a file — the copy-paste block is the deploy interface, and residual exposure is scrollback and CI logs, which the owner accepts. Image names come from `name: opennourish` in `docker-compose.yml`, not from the checkout directory.
...
**The suite needs `typst` on PATH.** 20 label tests shell out to it and fail `500 == 200` without it — that is the first thing a bare runner hits. CI downloads the same build the Dockerfile does and installs `fonts-liberation`, because `opennourish/typst_utils.py` pins `Liberation Sans`.
...
The two ruff gates are only meaningful together with `ruff.toml`: `select` freezes the rule families this tree is clean under (`E4`, `E7`, `E9`, `F`, plus `DTZ003` alone as of 2026-09-25 — the rest of `DTZ` is deliberately not selected, see the comment there) and `exclude = ["*.md"]` keeps ruff ≥0.16 out of Markdown code fences. Treat both keys as part of the gate contract, not as configuration taste.
...
- **`deploy_truenas.sh` keeps printing the full TrueNAS YAML with its secrets — owner decision 2026-09-23, "I want this printed, keep it as it is".** Do not redact it, write it to a file instead, or gate it behind a flag. The other M6.3 fixes (sourced `.env`, dropping `FLASK_DEBUG`, the compose `name:`) were proposed separately and landed 2026-09-24 without touching that output.
- **Every deploy is allowed to look like an update — owner decision 2026-09-25.** The wall-clock `OPENNOURISH_BUILD_DATE` stays. Because it is baked into `/app/BUILD_INFO` and `org.opencontainers.image.created`, every run yields a new digest and TrueNAS flags the app even on an unchanged tree. Deriving the stamp from the commit date instead would make no-op pushes byte-identical and silent, which was weighed and declined: always knowing which build is running outranks a quiet badge. Do not "optimise" the stamp away; judge a pending update with `BUILD_INFO` against the registry's manifest labels, and see `DEV-README.md` § Step 4.
