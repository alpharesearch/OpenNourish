...
- `deploy_truenas.sh:20` does an unquoted `export $(cat .env | ...)`, which word-splits values containing spaces and exports every unrelated key in `.env`. Its closing `cat <<EOF` prints the generated TrueNAS custom-app YAML — **which is the deploy artifact itself, and its `SECRET_KEY`/`ENCRYPTION_KEY`/`MAIL_*` lines are load-bearing** (see Work Guidance: mail comes from the environment until the DB says otherwise). **The owner has ruled that this print stays exactly as it is (2026-09-23):** do not silence, redact, or redirect it to a file — the copy-paste block is the deploy interface. Residual exposure is scrollback and CI logs, which the owner accepts; what still needs fixing there is `FLASK_DEBUG`, which no code reads. It also derives image names from the checkout directory name, so it only works from a directory called `opennourish`. Fix plan: `upgrade-research/PLAN.md` M6.3.
...
- **`deploy_truenas.sh` keeps printing the full TrueNAS YAML with its secrets — owner decision 2026-09-23, "I want this printed, keep it as it is".** Do not redact it, write it to a file instead, or gate it behind a flag; propose the other M6.3 fixes (quoted `.env` load, dropping `FLASK_DEBUG`, the compose `name:`) separately.
