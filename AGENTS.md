...
- Cross-user reads require an accepted `Friendship` — see `opennourish/AGENTS.md`, whose Work Guidance lists the current violations (unauthorised friend-diary read, an unguarded recipe label SVG, an ownership test that only rejects deleted owners, `undo` restoring with a session-supplied `user_id`). Fix them where you meet them; do not copy them.
