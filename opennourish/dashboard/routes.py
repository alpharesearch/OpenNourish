...
def index(log_date_str=None):
    ...
    daily_logs = DailyLog.query.filter_by(
        user_id=current_user.id, log_date=date_obj
    ).all()
    ...
    projected_dates, projected_weights, trending_away, at_goal_and_maintaining = (
        calculate_weight_projection(current_user)
    )
    days_to_goal = None
    goal_date_str = None
    ...
    last_completed_fast = (
        ...
        .order_by(FastingSession.end_time.desc())
        .first()
    )
