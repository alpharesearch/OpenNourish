...
def dashboard(username, log_date_str=None):
    ...
    daily_logs = DailyLog.query.filter_by(user_id=friend_user.id, log_date=date_obj).all()
    
    totals = calculate_nutrition_for_items(daily_logs)

    exercise_logs = ExerciseLog.query.filter_by(user_id=friend_user.id, log_date=date_obj).all()
...
def diary(username, log_date_str=None):
    ...
    daily_logs = DailyLog.query.filter_by(user_id=friend_user.id, log_date=log_date).all()
    
    exercise_logs = ExerciseLog.query.filter_by(user_id=friend_user.id, log_date=log_date).all()
    calories_burned = sum(log.calories_burned for log in exercise_logs)
