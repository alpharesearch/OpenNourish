...
def log_exercise():
    ...
    if form.validate_on_submit():
        ...
        if form.activity.data:
            ...
            calories_burned = calculate_calories_burned(
                activity, form.duration_minutes.data, user_weight_kg
            )
            activity_id = activity.id
        else:
            calories_burned = (
                form.calories_burned.data
                if form.calories_burned.data is not None
                else 0
            )
    ...
    for item in logs.items:
        ...
        edit_form.activity.query = ExerciseActivity.query.order_by(
    ...
    weekly_progress = {"calories_burned": 0, "exercises": 0, "minutes": 0}
    ...
    if user_goal:
        ...
        if weekly_logs:
            weekly_progress["calories_burned"] = sum(
                log.calories_burned for log in weekly_logs
            )
            weekly_progress["exercises"] = len(weekly_logs)
            weekly_progress["minutes"] = sum(
                log.duration_minutes for log in weekly_logs
            )
...
def edit_exercise(log_id):
    ...
    if form.validate_on_submit():
        ...
        if form.activity.data:
            ...
            exercise_log.calories_burned = calculate_calories_burned(
                activity, form.duration_minutes.data, user_weight_kg
            )
        else:
