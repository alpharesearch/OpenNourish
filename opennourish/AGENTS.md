...
- Shared services: `utils.py` (nutrition, portions, unit conversion, BMR, encryption helpers), `typst_utils.py` (nutrition-label PDF/SVG via the `typst` binary), `time_utils.py` (timezone helpers, the naive-UTC `utcnow_naive()`, Jinja filters), `decorators.py`, `context_processors.py`.
