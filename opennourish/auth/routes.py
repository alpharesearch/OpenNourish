...
def register():
    ...
    if form.validate_on_submit():
        ...
        if user.is_admin:
            flash(
                "Congratulations, you are now a registered user and have been granted administrator privileges!",
                "success",
            )
        else:
...
def verify_email(token):
    ...
    db.session.refresh(
        user
    )  # Refresh the user object to reflect the updated is_verified status
