...
class EmailSettingsForm(FlaskForm):
    MAIL_CONFIG_SOURCE = RadioField(
        "Configuration Source",
        choices=[("database", "Database"), ("environment", "Environment Variables")],
        default="database",
        validators=[DataRequired()],
    )
    MAIL_SERVER = StringField("Mail Server", validators=[Optional()])
    MAIL_PORT = IntegerField("Mail Port", validators=[Optional()])
    MAIL_SECURITY_PROTOCOL = RadioField(
        "Security Protocol",
        ...
        if self.MAIL_CONFIG_SOURCE.data == "database":
            ...
            if self.ENABLE_PASSWORD_RESET.data or self.ENABLE_EMAIL_VERIFICATION.data:
                if not self.MAIL_SERVER.data:
                    self.MAIL_SERVER.errors.append(
                        "Mail Server is required when enabling email features."
                    )
                if not self.MAIL_PORT.data:
                    self.MAIL_PORT.errors.append(
                        "Mail Port is required when enabling email features."
                    )
                if not self.MAIL_FROM.data:
                    self.MAIL_FROM.errors.append(
                        "Mail From Address is required when enabling email features."
                    )
