...
def test_register_admin_user(client):
    ...
    client.post(
        '/auth/register',
        data={'username': 'admin', 'email': 'admin@example.com', 'password': 'password', 'password2': 'password'},
...
def test_retroactive_admin_grant_on_login(client):
    ...
    client.post(
        '/auth/register',
        data={'username': 'regularuser', 'email': 'regularuser@example.com', 'password': 'password', 'password2': 'password'},
        follow_redirects=True
    )
    ...
    user.is_admin = False
    user.username = 'admin' # Change username to match the one that gets retroactive admin
    db.session.commit()
