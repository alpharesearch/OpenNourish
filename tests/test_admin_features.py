...
def test_first_registered_user_is_admin(client, monkeypatch):
    ...
    assert (
        b"Congratulations, you are now a registered user and have been granted administrator privileges!"
        in response.data
    )
...
def test_manage_admin_cli_command(app_with_db):
    ...
    result = runner.invoke(
        args=["user", "manage-admin", "cli_user", "--action", "grant"]
    )
    ...
    result = runner.invoke(
        args=["user", "manage-admin", "cli_user", "--action", "revoke"]
    )
    ...
    result = runner.invoke(
        args=["user", "manage-admin", "no_such_user", "--action", "grant"]
    )
...
def test_manage_key_user_cli_command(app_with_db):
    ...
    result = runner.invoke(
        args=["user", "manage-key-user", "cli_key_user", "--action", "grant"]
    )
    assert "Successfully granted" in result.output
    with app_with_db.app_context():
    ...
    result = runner.invoke(
        args=["user", "manage-key-user", "cli_key_user", "--action", "revoke"]
    )
    assert "Successfully revoked" in result.output
    with app_with_db.app_context():
        user = User.query.filter_by(username="cli_key_user").first()
    ...
    # Test with a non-existent user
    result = runner.invoke(
        args=["user", "manage-key-user", "no_such_user", "--action", "grant"]
    )
    assert "Error: User 'no_such_user' not found" in result.output
