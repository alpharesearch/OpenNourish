...
def admin_client_two_users(app_with_db):
    ...
    with app_with_db.app_context():
        ...
        regular_user = User(
            username="user", email="user@example.com", has_completed_onboarding=True
        )
...
def test_non_admin_cannot_access_user_management_page(client):
    ...
    with client.application.app_context():
        ...
        db.session.commit()
    client.post(
        "/auth/login", data={"username_or_email": "nonadmin", "password": "password"}
    )
...
def test_admin_cannot_disable_own_account(admin_client_two_users):
    ...
    response = client.post(
        f"/admin/users/{admin_user.id}/disable", follow_redirects=True
    )
...
def test_navigation_links(admin_client_two_users):
    ...
    client.post(
        "/auth/login", data={"username_or_email": "user", "password": "password"}
    )
    response = client.get("/dashboard/")
