...
def create_test_user(username, password='password', is_verified=True, is_private=False):
    user = User(username=username, email=f'{username}@example.com', is_verified=is_verified, is_private=is_private)
...
def test_is_private_checkbox_on_settings_page(auth_client_public_user):
    ...
    assert b'When enabled, other users will not be able to find you in searches or send you new friend requests. This will not affect your existing friendships.' in response.data
...
def test_save_is_private_setting(auth_client_public_user):
    ...
    response = client.post(url_for('settings.settings'), data={
        'email': user.email,
...
def test_cannot_send_friend_request_to_private_user(auth_client_public_user):
    ...
    response = client.post(url_for('friends.add_friend'), data={'username': 'hidden_user'}, follow_redirects=True)
    assert response.status_code == 200
    assert b'User not found.' in response.data # Should behave as if user not found

    with client.application.app_context():
        # Verify no friendship was created
        friendship = Friendship.query.filter_by(requester_id=requester_user_in_context.id, receiver_id=private_user_in_context.id).first()
...
def test_can_send_friend_request_to_public_user(auth_client_public_user):
    ...
    response = client.post(url_for('friends.add_friend'), data={'username': 'visible_user'}, follow_redirects=True)
    assert response.status_code == 200
    assert b'Friend request sent to visible_user.' in response.data

    with client.application.app_context():
        # Verify friendship was created
        friendship = Friendship.query.filter_by(requester_id=requester_user_in_context.id, receiver_id=public_user_in_context.id).first()
...
def test_existing_friendship_unaffected_by_private_mode(app_with_db):
    with app_with_db.app_context():
        ...
        existing_friendship = Friendship.query.filter_by(requester_id=user1.id, receiver_id=user2.id, status='accepted').first()
