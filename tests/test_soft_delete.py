...
def single_user(app_with_db):
    with app_with_db.app_context():
        user = User(
            username="single_user_soft_delete",
            email="single_user_soft_delete@example.com",
        )
...
def two_users_with_friendship(app_with_db):
    with app_with_db.app_context():
        user_a = User(
            username="user_a_soft_delete", email="user_a_soft_delete@example.com"
        )
        user_a.set_password("password_a")
        user_b = User(
            username="user_b_soft_delete", email="user_b_soft_delete@example.com"
        )
...
def test_delete_anonymizes_item_and_hides_from_owner(client, two_users_with_friendship):
    ...
    client.post(
        "/auth/login",
        data={"username_or_email": "user_a_soft_delete", "password": "password_a"},
...
def test_soft_delete_preserves_other_users_data_integrity(
    client, two_users_with_friendship
):
...
def test_ui_gracefully_displays_orphaned_items_single_user(client, single_user):
    ...
    client.post(
        "/auth/login",
        data={"username_or_email": "single_user_soft_delete", "password": "password"},
        follow_redirects=True,
    )
