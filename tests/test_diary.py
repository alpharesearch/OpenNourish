...
def test_add_usda_food_to_diary(auth_client):
    ...
    with auth_client.application.app_context():
        ...
        db.session.add(
            FoodNutrient(fdc_id=10001, nutrient_id=1008, amount=150.0)
        )  # 150 kcal per 100g
        # Add the mandatory 1-gram portion for the test
        gram_portion = UnifiedPortion(
            fdc_id=10001, portion_description="gram", gram_weight=1.0
        )
...
def test_add_usda_food_with_portion(auth_client):
    ...
    with auth_client.application.app_context():
        ...
        portion = UnifiedPortion(
            fdc_id=10002, portion_description="slice", gram_weight=28.0
        )
...
def test_add_my_food_with_portion(auth_client):
    ...
    with auth_client.application.app_context():
        user = User.query.filter_by(username="testuser").first()
        my_food = MyFood(
            user_id=user.id, description="My Custom Nuts", calories_per_100g=600
        )
        db.session.add(my_food)
        db.session.commit()
        my_food_id = my_food.id
        ...
        my_portion = UnifiedPortion(
            my_food_id=my_food_id, portion_description="handful", gram_weight=40.0
        )
...
def test_update_diary_entry(auth_client):
    ...
    with auth_client.application.app_context():
        ...
        portion2 = UnifiedPortion(
            my_food_id=my_food.id, portion_description="serving", gram_weight=50.0
        )
    ...
    response = auth_client.post(
        ...
        data={"amount": 2, "portion_id": portion2_id},
        follow_redirects=True,
    )
