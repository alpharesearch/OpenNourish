...
def setup_friend_data(friend_user):
    ...
    friend_recipe = Recipe(
        ...
        name="Friend's Recipe",
        instructions="Friend's instructions",
        servings=2,
        is_public=False,
    )
    ...
    friend_food = MyFood(
        user_id=friend_user.id, description="Friend's Food", calories_per_100g=100
    )
...
def test_search_friends_content(auth_client_with_friendship):
    ...
    response = client.post(
        "/search/", data={"search_term": "Friend"}, follow_redirects=True
    )
    assert response.status_code == 200
    ...
    response = client.post(
        "/search/",
        data={"search_term": "Friend", "search_friends": "true"},
...
def test_copy_meal_from_friend_diary(auth_client_with_friendship):
    ...
    with client.session_transaction() as session:
        ...
        assert (
            flashes[0][1]
            == f"Successfully copied {meal_name} from {friend_user.username}'s diary."
        )
    ...
    with client.application.app_context():
        user_logs = DailyLog.query.filter_by(
            user_id=test_user.id, log_date=log_date, meal_name=meal_name
        ).all()
