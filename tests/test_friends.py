...
def create_test_user(username, password="password", is_verified=True):
    user = User(
        username=username, email=f"{username}@example.com", is_verified=is_verified
    )
...
def test_send_friend_request(auth_client_with_user):
    ...
    response = test_client.post(
        "/friends/add", data={"username": "friend"}, follow_redirects=False
    )
    ...
    friendship = Friendship.query.filter_by(
        requester_id=test_user.id, receiver_id=user2.id
    ).first()
...
def test_send_friend_request_unverified_user(client):
    ...
    friendship = Friendship.query.filter_by(
        requester_id=unverified_user.id, receiver_id=recipient_user.id
    ).first()
...
def test_send_friend_request_verified_user(client):
    ...
    with client.application.test_request_context():
        response = client.post(
            url_for("friends.add_friend"),
            data={"username": "recipient_verified"},
            follow_redirects=True,
        )
    assert response.status_code == 200
    ...
    friendship = Friendship.query.filter_by(
        requester_id=verified_user.id, receiver_id=recipient_user.id
    ).first()
    assert friendship is not None
    assert friendship.status == "pending"
...
def test_accept_friend_request(auth_client_with_user):
    ...
    friend_request = Friendship(
        requester_id=user2.id, receiver_id=test_user.id, status="pending"
    )
    ...
    response = test_client.post(
        f"/friends/request/{friend_request.id}/accept", follow_redirects=True
    )
    assert response.status_code == 200
...
def test_decline_friend_request(auth_client_with_user):
    ...
    response = test_client.post(
        f"/friends/request/{friend_request.id}/decline", follow_redirects=True
    )
    assert response.status_code == 200
...
def test_remove_friend(auth_client_with_user):
    ...
    friendship = Friendship(
        requester_id=test_user.id, receiver_id=user2.id, status="accepted"
    )
    ...
    response = test_client.post(
        f"/friends/friendship/{user2.id}/remove", follow_redirects=True
    )
    assert response.status_code == 200
