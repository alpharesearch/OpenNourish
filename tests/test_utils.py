import pytest
from models import db, Food, Nutrient, FoodNutrient, User, SystemSetting, MyFood, Recipe, RecipeIngredient, DailyLog
...
def client_with_usda_food(auth_client_utils):
    ...
    with client.application.app_context():
        ...
        nutrients_to_add = [
            (1008, 'Energy', 'kcal', 100.0), (1004, 'Total lipid (fat)', 'g', 5.0),
            (1258, 'Fatty acids, total saturated', 'g', 2.0), (1257, 'Fatty acids, total trans', 'g', 0.1),
            (1253, 'Cholesterol', 'mg', 10.0), (1093, 'Sodium, Na', 'mg', 200.0),
            (1005, 'Carbohydrate, by difference', 'g', 15.0), (1079, 'Fiber, total dietary', 'g', 3.0),
            (2000, 'Total Sugars', 'g', 10.0), (1235, 'Sugars, added', 'g', 5.0),
            (1003, 'Protein', 'g', 8.0), (1110, 'Vitamin D (D2 + D3)', 'mcg', 2.0),
            (1087, 'Calcium, Ca', 'mg', 260.0), (1089, 'Iron, Fe', 'mg', 8.0),
        ...
        for nutrient_id, name, unit, amount in nutrients_to_add:
            ...
            db.session.add(FoodNutrient(fdc_id=usda_food.fdc_id, nutrient_id=nutrient_id, amount=amount))
...
def client_with_my_food(auth_client_utils):
    ...
    with client.application.app_context():
        my_food = MyFood(
            user_id=user_id, description='Test MyFood', calories_per_100g=150,
...
def client_with_recipe(auth_client_utils):
    ...
    with client.application.app_context():
        ...
        ingredient = RecipeIngredient(recipe_id=recipe.id, amount_grams=100, my_food_id=my_food_for_recipe.id)
...
def test_calculate_weekly_nutrition_summary_with_logs(user_for_utils):
    ...
    with client.application.app_context():
        ...
        log1 = DailyLog(user_id=user_id, log_date=date(2025, 1, 1), my_food_id=my_food.id, amount_grams=100) # 100 kcal
        log2 = DailyLog(user_id=user_id, log_date=date(2025, 1, 1), my_food_id=my_food.id, amount_grams=50)  # 50 kcal
        log3 = DailyLog(user_id=user_id, log_date=date(2025, 1, 2), my_food_id=my_food.id, amount_grams=200) # 200 kcal
        log4 = DailyLog(user_id=user_id, log_date=date(2025, 1, 3), my_food_id=my_food.id, amount_grams=100) # 100 kcal
