...
def users(app_with_db):
    with app_with_db.app_context():
        user1 = User(username='testuser1', email='testuser1@example.com', is_verified=False, is_private=True, has_completed_onboarding=False)
        user1.set_password('password')
        user2 = User(username='testuser2', email='testuser2@example.com', is_verified=True, is_private=False, has_completed_onboarding=True)
...
def users2(app_with_db):
    with app_with_db.app_context():
        user1 = User(username='testuser3', email='testuser3@example.com', is_verified=False, is_private=True, has_completed_onboarding=True)
        user1.set_password('password')
        user2 = User(username='testuser4', email='testuser4@example.com', is_verified=True, is_private=False, has_completed_onboarding=True)
        user2.set_password('password')
        db.session.add_all([user1, user2])
        db.session.commit()
        return [user1.id, user2.id]
...
def test_verify_user(admin_client, users):
    ...
    response = client.post(url_for('admin.verify_user', user_id=user_to_verify_id), follow_redirects=True)
    assert response.status_code == 200
    with client.application.app_context():
...
def test_unverify_user(admin_client, users):
    ...
    response = client.post(url_for('admin.unverify_user', user_id=user_to_unverify_id), follow_redirects=True)
    assert response.status_code == 200
    with client.application.app_context():
...
def test_make_user_public(admin_client, users):
    ...
    response = client.post(url_for('admin.make_user_public', user_id=user_to_make_public_id), follow_redirects=True)
    assert response.status_code == 200
    with client.application.app_context():
...
def test_make_user_private(admin_client, users):
    ...
    response = client.post(url_for('admin.make_user_private', user_id=user_to_make_private_id), follow_redirects=True)
    assert response.status_code == 200
    with client.application.app_context():
...
def test_complete_onboarding(admin_client, users):
    ...
    response = client.post(url_for('admin.complete_onboarding', user_id=user_to_complete_id), follow_redirects=True)
    assert response.status_code == 200
    with client.application.app_context():
...
def test_reset_onboarding(admin_client, users):
    ...
    response = client.post(url_for('admin.reset_onboarding', user_id=user_to_reset_id), follow_redirects=True)
    assert response.status_code == 200
    with client.application.app_context():
