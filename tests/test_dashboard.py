...
def test_dashboard_weight_projection_display(auth_client_onboarded):
    ...
    with client.application.app_context():
        ...
        checkin3 = CheckIn(user_id=user.id, checkin_date=today - timedelta(weeks=4), weight_kg=78.0, body_fat_percentage=20.0, waist_cm=90.0)
        checkin4 = CheckIn(user_id=user.id, checkin_date=today - timedelta(weeks=2), weight_kg=76.0, body_fat_percentage=19.0, waist_cm=89.0)
        checkin5 = CheckIn(user_id=user.id, checkin_date=today, weight_kg=75.0, body_fat_percentage=18.0, waist_cm=88.0)
        ...
        for i in range(14):
            ...
            db.session.add(DailyLog(user_id=user.id, log_date=log_date, fdc_id=food.fdc_id, amount_grams=100))
            db.session.add(ExerciseLog(user_id=user.id, log_date=log_date, calories_burned=100, duration_minutes=30))
        db.session.commit()
...
@pytest.mark.parametrize("time_range,expected_checkins", [
    ('1_month', 2),
    ('3_month', 4),
    ('6_month', 7),
    ('1_year', 12),
...
def test_dashboard_time_range_filter(auth_client_onboarded, time_range, expected_checkins):
    """
