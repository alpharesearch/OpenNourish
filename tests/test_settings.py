...
def test_change_password(client, auth_client):
    ...
    with auth_client.application.app_context():
        ...
        assert not user.check_password(
            "password"
        )  # Assuming initial password was 'password'
...
def test_set_timezone_no_timezone_provided(auth_client_with_user):
    ...
    with client.application.test_request_context():
        response = client.post(
            url_for("settings.set_timezone", _external=False), json={}
        )
...
def test_restart_onboarding(auth_client_with_user):
    ...
    with client.application.test_request_context():
        response = client.post(
            url_for("settings.restart_onboarding", _external=False),
...
def test_delete_account_success(auth_client_with_user):
    ...
    with client.application.app_context():
        ...
        db.session.commit()
        friendship_req = Friendship(
            requester_id=user.id, receiver_id=friend_user_1.id, status="accepted"
        )
        friendship_rec = Friendship(
            requester_id=friend_user_2.id, receiver_id=user.id, status="accepted"
        )
    ...
    with client.application.app_context():
        ...
        assert (
            db.session.get(MyMeal, my_meal_id) is None
        )  # MyMeal should be deleted due to cascade
...
def test_delete_account_incorrect_password(auth_client_with_user):
    ...
    )  # Should render the same page with error
