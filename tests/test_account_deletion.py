...
def setup_shared_content(app_with_db, user_a, user_b):
    with app_with_db.app_context():
        ...
        my_food_a = MyFood(
            user_id=user_a_obj.id, description="Alice's Chili", calories_per_100g=100
        )
        recipe_a = Recipe(
        ...
        daily_log_b = DailyLog(
            user_id=user_b_obj.id,
            log_date=date(2025, 1, 1),
            meal_name="Lunch",
            my_food_id=my_food_a.id,
            amount_grams=200,
        )
        ...
        recipe_b = Recipe(
            user_id=user_b_obj.id, name="Bob's Stew", servings=4, calories_per_100g=150
        )
        db.session.add(recipe_b)
        db.session.commit()
        recipe_ingredient_b = RecipeIngredient(
            recipe_id=recipe_b.id, recipe_id_link=recipe_a.id, amount_grams=100
        )
...
def test_account_deletion_anonymizes_and_deletes_correctly(app_with_db, client, user_a):
    with app_with_db.app_context():
        ...
        daily_log_a = DailyLog(
            user_id=user_a_obj.id,
            log_date=date(2025, 1, 1),
            meal_name="Breakfast",
            amount_grams=100,
        )
        exercise_log_a = ExerciseLog(
            user_id=user_a_obj.id, duration_minutes=30, calories_burned=300
        )
        my_meal_a = MyMeal(user_id=user_a_obj.id, name="UserA's Meal")
        db.session.add_all(
            [user_goal_a, check_in_a, daily_log_a, exercise_log_a, my_meal_a]
        )
        db.session.commit()
        my_meal_item_a = MyMealItem(
            my_meal_id=my_meal_a.id, fdc_id=12345, amount_grams=100
        )
        db.session.add(my_meal_item_a)

        # Setup shared content for User A
        my_food_a = MyFood(
            user_id=user_a_obj.id, description="UserA's Food", calories_per_100g=100
        )
        recipe_a = Recipe(
            user_id=user_a_obj.id,
            name="UserA's Recipe",
...
def test_deletion_does_not_break_other_users_data(
    app_with_db, client, user_a, user_b, setup_shared_content
):
    with app_with_db.app_context():
        user_a_obj = db.session.get(User, user_a)
        ...
        retrieved_recipe_ingredient_b = db.session.get(
            RecipeIngredient, recipe_ingredient_b_id
        )
        assert retrieved_recipe_ingredient_b is not None
        assert retrieved_recipe_ingredient_b.recipe_id_link == orphaned_recipe.id


def test_ui_handles_orphaned_content_gracefully(
    app_with_db, client, user_a, user_b, setup_shared_content
):
    with app_with_db.app_context():
        user_a_obj = db.session.get(User, user_a)
        ...
        friendship = Friendship(
            requester_id=user_a_obj.id, receiver_id=user_b_obj.id, status="accepted"
        )
        ...
        diary_response = client.get(
            url_for("diary.diary", log_date_str=date(2025, 1, 1).isoformat())
        )
        ...
        search_response = client.get(
            url_for("search.search", search_friends=True, search_term="Alice's Pie")
        )
...
def test_delete_account_fails_with_incorrect_password(app_with_db, client, user_a):
    with app_with_db.app_context():
        ...
        assert (
            b"Confirm Account Deletion" in response.data
        )  # Should remain on the confirmation page
