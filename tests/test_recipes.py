...
def unverified_user_client_for_recipes(app_with_db):
    with app_with_db.app_context():
        user = User(
            username="unverified_recipe_user",
            email="unverified_recipe@example.com",
            is_verified=False,
        )
...
def verified_user_client_for_recipes(app_with_db):
    with app_with_db.app_context():
        user = User(
            username="verified_recipe_user",
            email="verified_recipe@example.com",
            is_verified=True,
        )
        user.set_password("password")
        db.session.add(user)
...
def test_recipe_creation_and_editing(auth_client_with_user):
    ...
    response = client.post(
        url_for("recipes.new_recipe"), data=new_recipe_data, follow_redirects=False
    )
...
def test_edit_recipe_public_unverified_user(unverified_user_client_for_recipes):
    ...
    with client.application.app_context():
        recipe = Recipe(
            user_id=user.id, name="Test Recipe", instructions="Test", is_public=False
        )
...
def test_recipe_nutrition_calculation(auth_client_with_user):
    ...
    with client.application.app_context():
        recipe = Recipe(
            user_id=user.id, name="Nutrient Test Recipe", instructions="Test"
        )
        db.session.add(recipe)
        db.session.commit()
        recipe_id = recipe.id
        ...
        db.session.add(
            FoodNutrient(fdc_id=20001, nutrient_id=1008, amount=100.0)
        )  # 100 kcal/100g
        db.session.commit()
...
def test_delete_recipe(auth_client_with_user):
    ...
    with client.application.app_context():
        recipe_to_delete = Recipe(
            user_id=user.id, name="Recipe to Delete", instructions="Delete me!"
        )
...
def test_add_ingredient_to_recipe(auth_client_with_user):
    ...
    with client.application.app_context():
        recipe = Recipe(
            user_id=user.id, name="Test Add Ingredient", instructions="Test"
        )
        db.session.add(recipe)
        db.session.commit()
        recipe_id = recipe.id
        ...
        gram_portion = UnifiedPortion(
            fdc_id=30001, portion_description="gram", gram_weight=1.0
        )
    ...
    with client.application.app_context():
        added_ingredient = RecipeIngredient.query.filter_by(
            recipe_id=recipe_id, fdc_id=30001
        ).first()
...
def test_delete_ingredient_from_recipe(auth_client_with_user):
    ...
    with client.application.app_context():
        recipe = Recipe(
            user_id=user.id, name="Test Delete Ingredient", instructions="Test"
        )
        db.session.add(recipe)
        db.session.commit()
        recipe_id = recipe.id
        ...
        ingredient = RecipeIngredient(
            recipe_id=recipe_id, fdc_id=40001, amount_grams=100
        )
        db.session.add(ingredient)
        db.session.commit()
...
def test_add_meal_to_recipe_as_ingredient(auth_client_with_user):
    ...
    with client.application.app_context():
        recipe = Recipe(
            user_id=user.id, name="Recipe with Meal Ingredient", instructions="Test"
        )
        db.session.add(recipe)
        db.session.commit()
        recipe_id = recipe.id
        ...
        meal_item_my_food = MyMealItem(
            my_meal_id=meal.id, my_food_id=my_food_item.id, amount_grams=50
        )
        ...
        with client.session_transaction() as session:
            ...
            assert (
                flashes[0][1]
                == '"My Meal for Recipe" (expanded) added to recipe Recipe with Meal Ingredient.'
            )
        ...
        ingredients = (
            ...
            .order_by(RecipeIngredient.amount_grams)
            .all()
        )
...
def test_user_cannot_edit_other_users_recipe(auth_client_user_two):
    ...
    with client.application.app_context():
        ...
        recipe_user_one = Recipe(
            user_id=user_one.id, name="User One's Recipe", instructions="Test"
        )
...
def test_user_cannot_delete_other_users_recipe(auth_client_user_two):
    ...
    with client.application.app_context():
        # Create a recipe belonging to user_one
        recipe_user_one = Recipe(
            user_id=user_one.id, name="User One's Recipe to Delete", instructions="Test"
        )
        db.session.add(recipe_user_one)
        db.session.commit()
        recipe_id = recipe_user_one.id
...
def test_recipes_list_view(auth_client_user_two):
    ...
    with client.application.app_context():
        ...
        recipe_one_public = Recipe(
            user_id=user_one.id,
            name="User One Public Recipe",
            instructions="Public",
            is_public=True,
        )
        recipe_one_private = Recipe(
            user_id=user_one.id,
            name="User One Private Recipe",
            instructions="Private",
            is_public=False,
        )

        # User two has a private recipe
        recipe_two_private = Recipe(
            user_id=user_two.id,
            name="User Two Private Recipe",
            instructions="My own",
            is_public=False,
        )
    ...
    assert (
        f'action="/recipes/{recipe_one_public_id}/delete"'.encode() not in response.data
    )
...
def test_add_recipe_as_ingredient_and_prevent_self_nesting(auth_client_with_user):
    ...
    with client.application.app_context():
        # Create two recipes
        recipe_main = Recipe(
            user_id=user.id, name="Main Recipe", instructions="Main instructions"
        )
        recipe_sub = Recipe(
            user_id=user.id, name="Sub Recipe", instructions="Sub instructions"
        )
        ...
        gram_portion = UnifiedPortion(
            recipe_id=recipe_sub.id, portion_description="gram", gram_weight=1.0
        )
        db.session.add(gram_portion)
        db.session.commit()
        gram_portion_id = gram_portion.id
    ...
    with client.application.app_context():
        ...
        main_recipe_ingredients = RecipeIngredient.query.filter_by(
            recipe_id=recipe_main.id
        ).all()
        ...
        assert response.status_code == 200
        assert (
            b"A recipe cannot be an ingredient of itself." in response.data
        )  # This is the expected flash message for self-nesting prevention

        # Verify Main Recipe does not have itself as an ingredient
        main_recipe_ingredients_after_attempt = RecipeIngredient.query.filter_by(
            recipe_id=recipe_main.id
        ).all()
        assert (
            len(main_recipe_ingredients_after_attempt) == 1
        )  # Should still be only the Sub Recipe
...
def test_auto_add_recipe_portion_from_servings(auth_client_with_user):
    """
    Tests the 'Create Portion from Servings' functionality using the sum of ingredient weights.
    """
    client, user = auth_client_with_user
    with client.application.app_context():
    ...
    assert response.status_code == 200
    assert (
        b"Portion created based on the sum of ingredient weights." in response.data
    ), "Flash message for using ingredient weights not found."

    # 3. Verify the new portion was created correctly
    with client.application.app_context():
        # Query for portions, excluding the default 1-gram portion if it exists
        portions = UnifiedPortion.query.filter(
            UnifiedPortion.recipe_id == recipe_id,
            UnifiedPortion.measure_unit_description == "serving",
        ).all()
...
def test_auto_add_recipe_portion_with_final_weight(auth_client_with_user):
    """
    Tests the 'Create Portion from Servings' functionality, prioritizing the final_weight_grams.
    """
    client, user = auth_client_with_user
    with client.application.app_context():
        # 1. Create a recipe with a final cooked weight
        recipe = Recipe(
            user_id=user.id,
            name="Final Weight Test Recipe",
            servings=5.0,
        ...
        # Add ingredients with a total raw weight of 600g (to ensure final weight is used)
        ing1 = RecipeIngredient(recipe_id=recipe_id, amount_grams=300)
        ing2 = RecipeIngredient(recipe_id=recipe_id, amount_grams=300)
        db.session.add_all([ing1, ing2])
        db.session.commit()

    # 2. Simulate the POST request, including the final_weight_grams in the form data
    response = client.post(
        f"/recipes/recipe/portion/auto_add/{recipe_id}",
        data={"servings": "5.0", "final_weight_grams": "500.0"},
        follow_redirects=True,
    )

    assert response.status_code == 200
    assert (
        b"Portion created based on the final cooked weight of the recipe."
        in response.data
    ), "Flash message for using final weight not found."

    # 3. Verify the new portion was created correctly based on final_weight_grams
    with client.application.app_context():
        portions = UnifiedPortion.query.filter(
            UnifiedPortion.recipe_id == recipe_id,
            UnifiedPortion.measure_unit_description == "serving",
...
def test_recipe_edit_ingredient_dropdown_1g_portion_uniqueness(auth_client_with_user):
    ...
    with client.application.app_context():
        # Create a recipe
        recipe = Recipe(
            user_id=user.id, name="Recipe with USDA Ingredient", instructions="Test"
        )
        db.session.add(recipe)
        db.session.commit()
        recipe_id = recipe.id
        ...
        usda_food = Food(
            fdc_id=usda_food_fdc_id, description="USDA Food for Recipe Test"
        )
        db.session.add(usda_food)
        db.session.commit()
    ...
    form_start_index = html_content.find(
        f'action="/recipes/recipe/ingredient/{ingredient_id}/update"'
    )
    ...
    with client.application.app_context():
        re_queried_one_gram_portion = db.session.get(
            UnifiedPortion, one_gram_portion_id
        )
...
def test_user_cannot_view_other_users_private_recipe(auth_client_user_two):
    ...
    with client.application.app_context():
        ...
        private_recipe = Recipe(
            user_id=user_one.id, name="User One's Private Recipe", is_public=False
        )
    ...
    response = client.get(
        url_for("recipes.view_recipe", recipe_id=recipe_id), follow_redirects=True
    )
    assert response.status_code == 200
...
def test_user_cannot_update_ingredient_in_other_users_recipe(auth_client_user_two):
    ...
    response = client.post(
        ...
        data={"amount": 200, "portion_id": portion_id},
        follow_redirects=True,
    )
    assert response.status_code == 200
    assert b"You are not authorized to modify this recipe." in response.data
