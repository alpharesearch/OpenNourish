...
def create_test_user(username, password="password", user_id=None, email=None):
    ...
    user = (
        User(username=username, id=user_id, email=email)
        if user_id
        else User(username=username, email=email)
    )
...
def test_admin_can_disable_and_enable_registration(admin_client):
    ...
    response = admin_client_instance.post(
        "/admin/settings", data={}, follow_redirects=False
    )
    ...
    response = admin_client_instance.get(
        response.headers["Location"], follow_redirects=True
    )
    assert response.status_code == 200
    ...
    register_redirect_response = admin_client_instance.get(
    ...
    login_page_response = admin_client_instance.get(
        register_redirect_response.headers["Location"]
    )
    ...
    login_response = admin_client_instance.post(
        "/auth/login",
        data={"username_or_email": admin_user.username, "password": "password"},
...
def test_admin_can_toggle_email_verification(admin_client):
    ...
    email_verification_setting = SystemSetting.query.filter_by(
    ...
    register_enabled_response = admin_client_instance.get(
        "/auth/register", follow_redirects=False
    )
    assert register_enabled_response.status_code == 200
    assert (
        b"New user registration is currently disabled."
        not in register_enabled_response.data
    )
    assert b"Register" in register_enabled_response.data

    # 9. Verify that a new user can be created
    new_user_response = admin_client_instance.post(
        "/auth/register",
        data={
            "username": "newuser",
