...
def logged_in_user_with_timezone(app_with_db):
    ...
    with app_with_db.app_context():
        user = User(username='testuser_tz', email='test_tz@example.com', timezone='America/New_York')
...
def logged_in_user_invalid_tz(app_with_db):
    ...
    with app_with_db.app_context():
        user = User(username='testuser_invalid_tz', email='test_invalid_tz@example.com', timezone='Invalid/Zone')
        user.set_password('password')
        db.session.add(user)
        db.session.commit()
...
def test_jinja_filters_with_timezone(app_with_db, logged_in_user_with_timezone):
    ...
    with app.test_request_context('/'):
        ...
        rendered_time = app.jinja_env.from_string('{{ my_date | user_time }}').render(my_date=utc_dt)
        rendered_date = app.jinja_env.from_string('{{ my_date | user_date }}').render(my_date=utc_dt)
...
def test_jinja_filters_invalid_tz_fallback(app_with_db, logged_in_user_invalid_tz, caplog):
