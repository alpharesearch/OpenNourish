...
def get_or_create_secret_key(persistent_path):
    ...
    # 3. If no key found, generate, save, and return a new one
        ...
        print(
            f"INFO: No SECRET_KEY found. Generating and saving a new one to {key_file_path}"
        )
...
class Config:
    ...
    SQLALCHEMY_DATABASE_URI = os.environ.get(
        "DATABASE_URL"
    ) or "sqlite:///" + os.path.join(persistent_dir, "user_data.db")
    SQLALCHEMY_BINDS = {
        "usda": os.environ.get("USDA_DATABASE_URL")
        or "sqlite:///" + os.path.join(persistent_dir, "usda_data.db")
    }
